...
export const GitGraphView: React.FC<{ data: GitData }> = ({ data }) => {
            ...
            renderFrameRef.current = requestAnimationFrame(() => {
                ...
                requestAnimationFrame(() => {
                    try {
                        ...
                        // Y 坐标映射：考虑展开的提交
                        // SVG 的 top 已经是 headerHeight，所以内容不需要再加 headerHeight
                        // 内容从 0 开始，对应第一行表格内容
    ...
    return (
        <div className="git-graph-view" style={{ height: '100%', display: 'flex', flexDirection: 'column', fontSize: '14px' }}>
            ...
            >
                {/* 图形 SVG - 绝对定位覆盖整个图形列 */}
                {/* SVG 从表头下方开始。将其置于 table 下方，确保 sticky 表头不会被覆盖 */}
                <svg
                    id="commitGraph"
                    ref={graphSvgRef}
                    style={{
                        position: 'absolute',
                        left: 0,
                        top: headerHeight,  // SVG 从表头下方开始，与 Description 列对齐
                        width: GRAPH_COLUMN_WIDTH,
                        height: totalHeight,  // 只包含表格内容高度，不包含表头
                        pointerEvents: 'none',
